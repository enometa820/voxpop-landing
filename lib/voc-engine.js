...
function buildCleanSentence(category, sanitizedComment, moderation) {
  // 스팸(운영 신호 없는 욕설·장난): 다룰 내용이 없으니 짧게 닫는다.
  if (moderation.exposureStatus === EXPOSURE_STATUS.SPAM) {
    return `${category}에 대한 장난성 입력이라 신호에서 제외했어요.`;
  }
  // 명예훼손·인신공격: 거친 표현은 못 보여주되, 손님이 강하게 속상했다는 마음과 주제는 전한다.
  if (moderation.hasDefamationRisk || moderation.hasPersonalAttack) {
    return `${category}에 대해 손님이 많이 속상해하신 한마디예요. 표현이 거칠어 안전하게 가렸지만, 한 번 살펴보시길 권해요.`;
  }
  // PII·욕설만 가린 경우: 운영 신호가 담긴 실제 문장을 보존(토큰만 제거).
