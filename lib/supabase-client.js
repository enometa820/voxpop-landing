...
// 매장 AI 메모리 조회 — store_profile(롤링 요약) + 최근 store_memory(인사이트).
// 사장 RLS로 자기 매장만. 무기명 절대선: 매장 단위 집계 기억만(손님 신원·원본 0).
export async function fetchStoreMemory(storeId) {
  const supabase = getSupabase();
  const [profileRes, memoryRes] = await Promise.all([
    supabase.from('store_profile').select('profile, updated_at').eq('store_id', storeId).maybeSingle(),
    supabase.from('store_memory').select('kind, content, created_at')
      .eq('store_id', storeId).order('created_at', { ascending: false }).limit(3),
  ]);
  if (profileRes.error) throw profileRes.error;
  if (memoryRes.error) throw memoryRes.error;
  return { profile: profileRes.data?.profile || '', insights: memoryRes.data || [] };
}
